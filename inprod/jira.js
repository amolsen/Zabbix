...
Jira.prototype.getTransitions = function (issue_key) {

	// const TicketTransitions = this.sendRequest('GET', '/rest/api/latest/issue/' + encodeURIComponent(issue_key) + '/transitions');
	const TicketTransitions = this.sendRequest('GET', '/rest/api/latest/issue/' + encodeURIComponent(issue_key) + '/transitions');
	Logger.log(Logger.INFO, 'Received transitions for issue ' + issue_key + ': ' + JSON.stringify(TicketTransitions.transition));

	if (this.request.getStatus() !== 200) {
		Logger.log(Logger.WARN, 'Failed to get transitions for issue ' + issue_key + '. HTTP code: ' + this.request.getStatus());
		throw 'Failed to get transitions for issue ' + issue_key + '. HTTP code: ' + this.request.getStatus();
	}

	if(TicketTransitions.transitions && Array.isArray(TicketTransitions.transitions) && TicketTransitions.transitions.length > 0) {
	...
	if(resolvedId){
		const body = {
		transition: {
			id: resolvedId // Assuming closeTransition is defined elsewhere
			}
		};
    }
	   return body;
...
Jira.prototype.closeIssue = function (issue_key, transition_body) {

    if (transition_body) {
	 this.sendRequest('POST', '/rest/api/latest/issue/' + encodeURIComponent(issue_key) + '/transitions', transition_body);
     Logger.log(Logger.INFO, 'Transitioned issue ' + issue_key + ' to resolved state.' + JSON.stringify(this.result));
     return this.result;
    } else {
        Logger.log(Logger.WARN, 'No transition body provided for closing the issue.');
        throw 'No transition body provided for closing the issue.';
    }
...
Jira.prototype.onUpdate = function (alert, dontUpdatePriority) {
    ...
    if (params.event_value === '0') {
        var transition_body = this.getTransitions(jira_issue_key);
        if (transition_body) {
            this.closeIssue(jira_issue_key, transition_body);
        }
