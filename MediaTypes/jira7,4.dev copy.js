...
Jira.prototype.sendRequest = function (method, path, data) {
	...
	//Anders
    //Zabbix.log(Logger.INFO, 'Response from Jira: ' + JSON.stringify(response));
...
Jira.prototype.onUpdate = function (alert, dontUpdatePriority) {
    ...
    const TicketTransitions = this.sendRequest('GET', '/rest/api/latest/issue/' + encodeURIComponent(jira_issue_key) + '/transitions');
	const MyTrans = TicketTransitions.transitions;
	const resolvedId = null;

for (var i = 0; i < MyTrans.length; i++) {
  var name = MyTrans[i].name;
  if (name && name.toLowerCase().indexOf("resolve") !== -1) {
    resolvedId = MyTrans[i].id;
    break;
  }
}
Logger.log(Logger.WARN, 'resolvedId ' + jira_issue_key + ': ' + resolvedId );

	//Logger.log(Logger.WARN, 'TicketTransitions ' + jira_issue_key + ': ' + TicketTransitions);
	//Logger.log(Logger.WARN, 'TicketTransitions.transitions' + jira_issue_key + ': ' + TicketTransitions.transitions);
	//Logger.log(Logger.WARN, 'Transitions for issue ' + jira_issue_key + ': ' + JSON.stringify(TicketTransitions));
	//Logger.log(Logger.WARN, 'Transitions for issue ' + jira_issue_key + ': ' + JSON.stringify(TicketTransitions.transitions));
	//const resolvedItem = TicketTransitions.transitions[0].find(item =>
  	//["Resolve", "Resolved", "Resolve this item", "Resolve this issue"].includes(item.name)
	//);
	//Logger.log(Logger.WARN, 'resolvedItem' + jira_issue_key + ': ' + resolvedItem );
	//Logger.log(Logger.WARN, 'Transitions JSON ' + jira_issue_key + ': ' + TicketTransitions);
	//Logger.log(Logger.WARN, 'Transitions JSON ' + jira_issue_key + ': ' + rawResponse.transitions);
	//const TicketTransitions = JSON.parse(response);
	//Logger.log(Logger.WARN, 'Transitions for issue ' + jira_issue_key + ': ' + JSON.stringify(TicketTransitions));
	//Logger.log(Logger.WARN, 'Transitions JSON ' + jira_issue_key + ': ' + response);
	//Logger.log(Logger.WARN, 'response JSON ' + jira_issue_key + ': ' + response);
	//	const resolvedItem = TicketTransitions.find(item =>["Resolve", "Resolved", "Resolve this item", "Resolve this issue"].includes(item.name));
	//const resolvedItem = response.transitions.find(item =>
	//  ["Resolve", "Resolved", "Resolve this item", "Resolve this issue"].includes(item.name)
	//);
	//Logger.log(Logger.WARN, 'resolvedItem' + jira_issue_key + ': ' + resolvedItem);
	//const resolvedId = resolvedItem ? resolvedItem.id : null;
	//Logger.log(Logger.WARN, 'Transitions for issue ' + jira_issue_key + ': ' + JSON.stringify(TicketTransitions.transitions));
	//Logger.log(Logger.WARN, 'Transitions for issue ' + jira_issue_key + ': ' + resolvedId);
	//Logger.log(Logger.WARN, 'Transitions for issue ' + jira_issue_key + ': ' + TicketTransitions.transitions.map(function (tr) { return tr.name.match(/Resolved|Resolve/); }).join(', '));
	//Logger.log(Logger.WARN, TicketTransitions.transitions.find(item => item.name === "Resolved"));
    //Zabbix.log(Logger.INFO, 'Transitions for issue ' + jira_issue_key + ': ' + JSON.stringify(TicketTransitions.transitions));
	//Zabbix.log(Logger.INFO, 'Transitions for issue ' + jira_issue_key + ': ' + TicketTransitions.transitions.map(function (tr) { return tr.name; }).join(', '));
    // data.state = 6;
	// data.incident_state = 6;
	}
    //const resolvedItem = data.find(item => item.name === "Resolved");
	//const resolvedId = resolvedItem ? resolvedItem.id : null;

    // if (TicketTransitions.transitions.length > 0) {
    //     Zabbix.log(Logger.INFO, 'Transitions for issue ' + jira_issue_key + ': ' + TicketTransitions.transitions.map(function (tr) { return tr.name; }).join(', '));
    //     var transition = TicketTransitions.transitions(function (tr) {
    //         return tr.name.match(/Resolved|Resolve/);
    // })
    //     // if (transition) {
    //     //     data.transition = {
    //     //         id: transition.id
    //     //     };
    //     // } else {
    //     //     Logger.log(Logger.WARN, 'No transition found for issue ' + jira_issue_key + '.');
    //     //     throw 'No transition found for issue ' + jira_issue_key + '.';
    //     // }
    //     this.sendRequest('POST', '/rest/api/latest/issue/' + encodeURIComponent(jira_issue_key) + '/transitions', data);
    // }
    //Anders END
