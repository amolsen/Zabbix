...
function closeJiraIncident(issueKey) {
    // Get available transitions
    var transitionsResponse = Jira.request('get', 'request/' + issueKey + '/transition');
    var transitions = transitionsResponse.response.values || [];

    // Find the transition for "Closed" or "Resolved"
    var closeTransition = transitions.find(function(tr) {
        return tr.name.match(/close|closed|resolve|resolved/i);
    });
            
    if (!closeTransition) {
        throw 'No "Close" or "Resolve" transition found for issue ' + issueKey;
    }
    ...
    Jira.request('post', 'request/' + issueKey + '/transition', transitionData);
}
kj
// Example usage after creating or updating the request
// Only call this when you want to close the incident, e.g. on recovery
if (params.event_recovery_value === '1') {
    closeJiraIncident(Jira.params.request_key);
}
